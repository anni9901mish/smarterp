...
const Purchase = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const supplierRes = await api.get(`/ledger/suppliers?companyId=${companyId}`);
  ...
  const handleRowChange = (index, field, value) => {
    ...
    if (field === "itemId") {
      ...
      if (selectedItem) {
        updatedRows[index].rate = Number(selectedItem.purchasePrice || 0);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Purchase creation failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold">Purchase Voucher</h1>
          <p className="text-slate-400">
            Add stock purchases and update supplier balance automatically.
          </p>
        </div>
      </div>
      ...
      >
        {loading ? "Saving Purchase..." : "Save Purchase Voucher"}
      </button>
    </div>
  );
};
